...
int main() {
	...
	// Store the file and path information: out << "///{" << path << endl;
	...
	if (query["confirmation"].length() != 0) {
		...
		if (conftype == "confirmation1") {
			string command("./ProcessApplication.exe " +
				path + " &");
			// The data file is the argument, and the // ampersand runs it as a separate process:
